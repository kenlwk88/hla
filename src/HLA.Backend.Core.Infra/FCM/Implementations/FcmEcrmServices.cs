...
{
    public class FcmEcrmServices : IFcmServices
    {
        private readonly ILogger<FcmEcrmServices> _logger;
        private readonly string _appName = "ECRM";
        public FcmEcrmServices(ILogger<FcmEcrmServices> logger, IConfiguration configuration)
        {
            _logger = logger;
            ...
            if (FirebaseApp.GetInstance(_appName) == null)
            {
                ...
                string fullPath = path + @"\ServiceAccount\" + configuration.GetSection("FCM:ServiceAccountFileName_ECRM").Value;
